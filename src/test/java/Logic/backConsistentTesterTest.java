 ...
 *
 */
public class backConsistentTesterTest {
	...
	public void testInspect() {
		AbstractLogic logic = new backConsistentTester(FIRST_LANGUAGE, SECOND_LANGUAGE);
		Word firstWord = new Word("Noun", "age", "eɪdʒ", "возраст");
		assertTrue("Logic's class incorrectly performs equality check", logic.inspect(firstWord, firstWord.getValue()));
		assertFalse("Logic's class incorrectly performs different case arguments", logic.inspect(firstWord, firstWord.getValue().toUpperCase()));
		assertFalse("Logic's class incorrectly performs equality check", logic.inspect(firstWord, "ability"));
	...
	public void testGetName() {
		AbstractLogic logic = new backConsistentTester(FIRST_LANGUAGE, SECOND_LANGUAGE);
		assertEquals("Logic's class returns incorrect name",logic.getName(), new StringBuilder().append("Translate from ").append(SECOND_LANGUAGE).append(" into ").append(FIRST_LANGUAGE).toString());
	}
